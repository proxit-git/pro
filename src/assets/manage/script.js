...
async function getIpDetails(ip) {
    try {
        const response = await fetch('/manage/my-ip', { method: 'POST', body: ip });
...
function downloadWarpConfigs(isAmnezia) {
    ...
    window.location.href = "/manage/get-warp-configs" + client;
}
...
async function updateWarpConfigs() {
    ...
    try {
        const response = await fetch('/manage/update-warp', { method: 'POST', credentials: 'include' });
        ...
        alert('✅ W configs updated successfully!');
    } catch (error) {
